...
import { DispatchType, EffectCallback } from "../types";
import { nextTick } from "../utils/dispatch";
import { effect } from "./index";

describe("dispatch: async", () => {
    defineSharedTests("async");
...
function defineSharedTests(dispatch: DispatchType): void {
    beforeEach(() => {
        setupDoMutation();
    });
    afterEach(() => {
    ...
    const doMutation = (fn: () => void) => doMutationImpl(fn, dispatch);
    ...
    describe("dependency tracking", () => {
        ...
        it("throws when a cycle is detected in a later execution", async () => {
            ...
            await expect(doMutation(() => (trigger.value = true))).rejects.toMatchInlineSnapshot(
